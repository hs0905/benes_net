...
int main()
{
 out_port_arr = (int *)malloc(size * sizeof(int));
 scanf("%d", &size);
 ...
 printf("%d",out_port_arr[i]);
 printf("\n");
 print_stage_switch_num(size);
}
/*=======================================================================================
define sub_function
=======================================================================================*/
...
void set_switch(int current_size) // switch_arr에 switch의 상태를 저장
{
 for (int i = 0; i < current_size/2; i++) // switch_arr의 행의 개수는 switch의 개수와 같음
 {
 if (path[2 * i][0] % 2 == 0)
 {
 IO_switch_arr[(int)path[2 * i][0] / 2][0] = SWITCH_BAR;
 }
 else
 {
 IO_switch_arr[(int)path[2 * i][0] / 2][0] = SWITCH_CROSS;
 }
 if (path[2 * i][1] % 2 == 0)
 {
 IO_switch_arr[(int)path[2 * i][1] / 2][1] = SWITCH_BAR;
 }
 else
 {
 IO_switch_arr[(int)path[2 * i][1] / 2][1] = SWITCH_CROSS;
 }
 }
}
